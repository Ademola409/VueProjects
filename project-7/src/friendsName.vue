<template>
    <li>
        <h2>{{friendName}}{{isFavorite ?'-'+ address :''}}</h2>
        ...
        <ul v-if="detailsAreVisible">
            ...
            <li><strong>Email:</strong>{{friendEmail}}</li>
        </ul>
...
<script>
export default {
    props:['friendName','friendEmail','friendPhone','isFavorite','address','id'],
    emits:['address-toggle','delete-contact'],
    data(){
        return{
    ...
    methods:{
        ...
        toggleAddress(){
          this.$emit('address-toggle',this.id)  
        },
        deleteContact(){
